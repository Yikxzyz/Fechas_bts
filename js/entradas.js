(function () {
  ...
  var activeZona     = "all";
  var maxPrecio      = Infinity; // sin límite por defecto
  var allListings    = [];
  ...
  function parsePrecio(str) {
    if (!str) return NaN;
    // elimina separadores de miles, letras, espacios y toma el número
  ...
  function filterListings() {
    return allListings.filter(function (row) {
      ...
      if (activeFecha !== "all") {
        ...
        if (fechas.indexOf(activeFecha) === -1) return false;
      }
      ...
      if (activeZona !== "all") {
        ...
        if (zonaRow.indexOf(activeZona.toLowerCase()) === -1) return false;
      }
  ...
  // ── Poblar chips de zona desde los datos ────────────────────────────────
  function populateZonas() {
    ...
    var container = document.getElementById("filter-zona");
    if (!container) return;

    zonas.forEach(function (zona) {
      var btn = document.createElement("button");
      btn.type = "button";
      btn.className = "chip chip--adv";
      btn.setAttribute("data-zona", zona);
      btn.textContent = zona;
      btn.addEventListener("click", function () {
        activeZona = zona;
        container.querySelectorAll("[data-zona]").forEach(function (el) {
  ...
  function resetAdvFilters() {
    ...
    // zona
    document.querySelectorAll("[data-zona]").forEach(function (el) {
      el.classList.toggle("active", el.getAttribute("data-zona") === "all");
    });
    // precio slider
    var slider = document.getElementById("filter-precio");
    var priceVal = document.querySelector(".adv-price-val");
  ...
  function initCantidadFilter() {
    var container = document.getElementById("filter-cantidad");
    if (!container) return;
    container.querySelectorAll("[data-cantidad]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        activeCantidad = btn.getAttribute("data-cantidad");
        container.querySelectorAll("[data-cantidad]").forEach(function (el) {
          el.classList.toggle("active", el === btn);
        });
        render();
      });
  ...
  // ── Inicializar filtro de zona (botón "Cualquiera") ──────────────────────
  function initZonaFilter() {
    var container = document.getElementById("filter-zona");
    if (!container) return;
    var allBtn = container.querySelector("[data-zona='all']");
    if (allBtn) {
      allBtn.addEventListener("click", function () {
        activeZona = "all";
        container.querySelectorAll("[data-zona]").forEach(function (el) {
          el.classList.toggle("active", el === allBtn);
        });
        render();
      });
    }
  ...
  function initPrecioFilter() {
    var slider   = document.getElementById("filter-precio");
    var priceVal = document.querySelector(".adv-price-val");
    var maxLabel = document.getElementById("adv-price-max");

    if (!slider) return;

    // Ajustar el max del slider cuando los datos estén cargados
    function updateSliderMax() {
      ...
      if (max > 0) {
        // redondear al 100k superior
        var rounded = Math.ceil(max / 100000) * 100000;
        slider.max   = rounded;
        slider.value = rounded;
        if (maxLabel) maxLabel.textContent = "$" + formatNum(rounded);
      }
    }
  ...
  function initAdvToggle() {
    var toggleBtn = document.getElementById("adv-toggle");
    var panel     = document.getElementById("adv-filters");
    ...
    var clearBtn = document.getElementById("adv-clear");
    if (clearBtn) {
      clearBtn.addEventListener("click", resetAdvFilters);
    }
  }
