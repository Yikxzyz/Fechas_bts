(function () {
  ...
  var COL = {
    estado:     0,   // A - Estado
    nombre:     3,   // D - Nombre Completo
    instagram:  4,   // E - Usuario de Instagram
    resideChile: 6,  // G - ¿Resides en Chile?
    ciudad:     7,   // H - Ciudad de preferencia para entrega presencial
    cantidad:   8,   // I - Cantidad de entradas
    fechas:     9,   // J - Fechas disponibles
    zonas:      10,  // K - Zonas disponibles
    precio:     11,  // L - Precio total
    whatsapp:   12,  // M - Número de contacto (WhatsApp)
    presencial: 18,  // S - ¿Dispuest@ a juntarse presencialmente?
  };
  ...
  var activeModalidad  = "all";    // "all" | "presencial" | "online"
  ...
  function filterListings() {
    return allListings.filter(function (row) {
      ...
      // Modalidad
      if (activeModalidad !== "all") {
        var reside = (row[COL.resideChile] || "").trim().toLowerCase();
        var presenc = (row[COL.presencial]  || "").trim().toLowerCase();
        var esPresencial = reside === "si" && presenc === "si";
        if (activeModalidad === "presencial" && !esPresencial) return false;
        if (activeModalidad === "online"     &&  esPresencial) return false;
      }
  ...
  function renderCard(row) {
    ...
    // Badges de ciudad, tipo de venta y verificado
    var resideRaw     = (row[COL.resideChile] || "").trim().toLowerCase();
    var presencialRaw = (row[COL.presencial]  || "").trim().toLowerCase();
    var ciudadRaw     = esc((row[COL.ciudad]  || "").trim());

    // Badge ciudad — solo si la persona indicó una ciudad
    var ciudadHtml = "";
    if (ciudadRaw) {
      ciudadHtml = '<span class="listing-ubicacion listing-ubicacion--presencial">📍 ' + ciudadRaw + '</span>';
    }

    // Badge tipo de venta — siempre visible
    var esPresencial = resideRaw === "si" && presencialRaw === "si";
    var tipoVentaHtml = esPresencial
      ? '<span class="listing-ubicacion listing-ubicacion--tipo-presencial">🤝 Presencial</span>'
      : '<span class="listing-ubicacion listing-ubicacion--online">🌐 Online</span>';
    ...
    return (
      ...
      '<div class="listing-header-right">' + ciudadHtml + tipoVentaHtml + '<span class="listing-badge">Verificado ✓</span></div>' +
      "</div>" +
  ...
  function render() {
    ...
    var hasAdv = activeFecha !== "all" || activeCantidad !== "all" || activeZonas.length > 0 || activeModalidad !== "all" || maxPrecio !== Infinity;
  ...
  function resetFilters() {
    ...
    document.querySelectorAll("[data-fecha]").forEach(function (el) {
      el.classList.toggle("active", el.getAttribute("data-fecha") === "all");
    });
    ...
    // precio input
    var input = document.getElementById("filter-precio");
    if (input) input.value = "";

    // modalidad chips
    document.querySelectorAll("[data-modalidad]").forEach(function (el) {
      el.classList.toggle("active", el.getAttribute("data-modalidad") === "all");
    });
  ...
  function initModalidadFilter() {
    document.querySelectorAll("[data-modalidad]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        activeModalidad = btn.getAttribute("data-modalidad");
        document.querySelectorAll("[data-modalidad]").forEach(function (el) {
  ...
  function initPrecioFilter() {
    ...
    // Formatea el valor mientras escribe: solo dígitos, con puntos de miles
    input.addEventListener("input", function () {
      // quitar todo lo que no sea dígito
      var raw = input.value.replace(/\D/g, "");
      // reformatear con puntos
      input.value = raw ? formatNum(parseInt(raw, 10)) : "";

      clearTimeout(debounce);
      debounce = setTimeout(function () {
        var n = parseInt(raw, 10);
        maxPrecio = (raw === "" || isNaN(n)) ? Infinity : n;
        // quitar preset activo si el usuario escribe manualmente
        document.querySelectorAll("[data-preset]").forEach(function (el) {
          el.classList.toggle("active", raw === "" && el.getAttribute("data-preset") === "all");
        });
        render();
      }, 300);
    });
    ...
    document.querySelectorAll("[data-preset]").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (preset === "all") {
          ...
          input.value = formatNum(maxPrecio);
        }
        document.querySelectorAll("[data-preset]").forEach(function (el) {
          el.classList.toggle("active", el === btn);
        });
